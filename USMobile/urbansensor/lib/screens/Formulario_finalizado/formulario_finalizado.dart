...
class FormularioFinalizado extends StatefulWidget {
  final String id;

  FormularioFinalizado({required this.id});

  @override
  _FormularioFinalizadoState createState() => _FormularioFinalizadoState();
}

class _FormularioFinalizadoState extends State<FormularioFinalizado> {
  late Future<Map<String, dynamic>?> _formularioData;
  ...
  void initState() {
    super.initState();
    _formularioData = fetchEncuestaDetalles(widget.id);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: MyAppBar(title: 'Finalizada', appBarHeight: appBarHeight),
      body: FutureBuilder<Map<String, dynamic>?>(
        future: _formularioData,
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: BotonGenerico(
                        ...
                        onPressed: () async {
                          try {
                            await cerrarIncidencia(data['ID'].toString());
                            Navigator.push(
                              context,
                              MaterialPageRoute(
                                builder: (context) => Enviadas(),
                              ),
                            );
                          } catch (e) {
                            print('Error al cerrar la incidencia: $e');
                          }
                        },
                      ),
                    ),
